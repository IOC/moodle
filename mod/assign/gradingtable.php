...
class assign_grading_table extends table_sql implements renderable {
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if ($filter == ASSIGN_FILTER_SUBMITTED) {
                ...
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 ...
                                 (s.timemodified > g.timemodified OR g.grade IS NULL OR g.grade < 0))';
                $params['submitted'] = ASSIGN_SUBMISSION_STATUS_SUBMITTED;
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $status, 'assign') . $deletedsubmission,
                                           array('class' => 'submissionstatus' .$status . $deletedclass));
