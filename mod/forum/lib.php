...
function forum_print_overview($courses,&$htmlarray) {
    ...
    if (count($trackingforums) > 0) {
        ...
        foreach ($trackingforums as $track) {
            ...
            $params[] = $track->id;
            $cm = get_coursemodule_from_instance('forum', $track->id);
            $groupmode = groups_get_activity_groupmode($cm);
            $groupids = false;
            if ($groupmode == VISIBLEGROUPS) {
                $groupdata = groups_get_course_data($track->course);
 ...
 * @return array
 */
function forum_get_discussions($cm, $forumsort="d.timemodified DESC", $fullpost=true, $unused=-1, $limit=-1, $userlastmodified=false, $page=-1, $perpage=0, $showfavs=false) {
    global $CFG, $DB, $USER;
    ...
    $sql = "SELECT DISTINCT $postdata, d.name, d.timemodified, d.usermodified, d.groupid, d.timestart, d.timeend, d.pinned, $allnames,
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (!has_capability('mod/forum:viewhiddentimedposts', $modcontext) and !$post->parent and $discussion->timestart) {
        ...
        $scheduled = html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('i/scheduled'),
    ...
    $output .= html_writer::tag('div', get_string('bynameondate', 'forum', $by) . $scheduled, array('class'=>'author',
                                                                                       'role' => 'heading',
                                                                                       'aria-level' => '2'));

    if (has_capability('mod/forum:viewhiddentimedposts', $modcontext) and !$post->parent and ($discussion->timestart or $discussion->timeend)) {
        $output .= html_writer::start_tag('div', array('class' => 'forum_discussion_info_scheduled'));
        if($discussion->timestart) {
            $output .= html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('i/scheduled')));
            $output .= html_writer::tag('span', userdate($discussion->timestart));
        }
        if($discussion->timeend) {
            $output .= html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('i/show')));
...
function forum_print_discussion_header(&$post, $forum, $group=-1, $datestring="",
    ...
    if ($post->favourite) {
        $strfavcount = '';
        $iconfav = html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('star', 'theme')));
    ...
    if (isset($post->numattachments)) {
        $iconattach = html_writer::empty_tag('img', array('src' => $OUTPUT->pix_url('t/attachment', 'mod_forum')));
...
// Get all the recent discussions we're allowed to see
    ...
    if (! $discussions = forum_get_discussions($cm, $sort, $fullpost, null, $maxdiscussions, $getuserlastmodified, $page, $perpage, $showfavs) ) {
